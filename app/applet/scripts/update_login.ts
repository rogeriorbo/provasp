...
const replacement = `  return (
    <div className="flex-1 w-full h-full flex flex-col items-center justify-center p-4 relative min-h-[calc(100vh-80px)]">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        />
        <div className={\`absolute inset-0 \${theme === 'dark' ? 'bg-zinc-950/80 backdrop-blur-sm' : 'bg-blue-900/60 backdrop-blur-sm'}\`} />
      </div>
      ...
      <motion.div 
        ...
        className={\`relative z-10 w-full max-w-lg p-8 sm:p-10 rounded-[32px] border \${theme === 'dark' ? 'bg-zinc-900/90 border-zinc-800 shadow-2xl shadow-black/50' : 'bg-white/95 border-white shadow-2xl shadow-blue-900/20'} backdrop-blur-xl\`}
      >
        {onBack && (
          <button 
            onClick={onBack}
        ...
        <div className="text-center mb-8 relative pt-2">
            <h2 className="text-2xl sm:text-3xl font-black mb-2">{mode === 'login' ? 'Área do Aluno' : 'Criar nova conta'}</h2>
